...
//                                                +#+#+#+#+#+   +#+           //
//   Created: 2020/02/18 19:49:00 by bshaquan          #+#    #+#             //
//   Updated: 2020/02/18 19:49:08 by bshaquan         ###   ########.fr       //
//                                                                            //
// ************************************************************************** //
	...
	$.getJSON("data.json", function(data) {
		...
		$("#next").click(function() {
			if (ants_out < ants_total) {
						...
						if (cy.getElementById(path.nodes[i]).style("opacity") == 1) {
							...
							for (i = 1; i < path.nodes.length; i++) {
								if (get_edge_opacity(path.nodes[i - 1], path.nodes[i]) == 1) {
									cy.getElementById(path.nodes[i]).style("opacity", 0.7);
									set_edge_opacity(path.nodes[i - 1], path.nodes[i], 0.2);
									break;
								}
							}
							break;
				...
				if (all_started == true) {
					cy.nodes('node[type = "start"]').style("opacity", 0.7);
				}
				rounds += 1;
				$("#rounds").text("Rounds: " + rounds);
			} else {
				cy.nodes().style("opacity", 0.7);
		...
		$("#reset").click(function() {
			...
			data.paths.forEach(function(path, p) {
				for (i = 0; i < path.nodes.length; i++) {
					cy.getElementById(path.nodes[i]).style("opacity", 0.7);
					set_edge_opacity(path.nodes[i], path.nodes[i + 1], 0.2);
				}
				ants_on_paths[p] = path.ants;
			});
			cy.nodes('node[type = "start"]').style("opacity", 1);
			cy.nodes('node[type = "end"]').style("opacity", 0.7);
